...
caseTitle: 일본어 IME 조합 중 Enter 키 입력 시 keyCode 229과 13의 중복 keydown 이벤트
description: "contenteditable 요소에서 일본어 IME 조합 중 Enter를 누르면 두 개의 keydown 이벤트가 발생합니다: 첫 번째는 keyCode 229(IME 처리), 두 번째는 keyCode 13(Enter). 이것은 단일 Enter 키 입력에 대해 이벤트 핸들러가 두 번 실행될 수 있습니다."
tags:
  - composition
  - ime
  - enter
...
`contenteditable` 요소에서 일본어 IME 조합 중 Enter를 누르면 두 개의 `keydown` 이벤트가 순차적으로 발생합니다:
...
2. 일본어 IME를 활성화합니다.
3. 일본어 텍스트 조합을 시작합니다 (예: 로마자로 "konnichiwa"를 입력하여 "こんにちは" 조합).
4. 조합이 활성화된 상태에서(한자 변환을 완료하기 전에) Enter를 누릅니다.
