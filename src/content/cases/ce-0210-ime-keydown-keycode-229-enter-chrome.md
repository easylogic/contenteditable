---
id: ce-0210-ime-keydown-keycode-229-enter-chrome
...
keyboard: Korean (IME)
caseTitle: Duplicate keydown events with keyCode 229 and 13 when pressing Enter during IME composition
description: "When pressing Enter during Korean IME composition in a contenteditable element, two keydown events fire: first with keyCode 229 (IME processing), then with keyCode 13 (Enter). This can cause event handlers to execute twice for a single Enter key press."
tags:
  - composition
  - ime
  - enter
...
- **Track last keyCode 229**: If you need to handle both events, track the sequence:
  ...
  element.addEventListener('keydown', (e) => {
    if (isComposing) {
      if (e.keyCode === 229) {
        lastKeyCode229 = e;
        return; // Ignore keyCode 229
      }
      
      // If actual keyCode follows keyCode 229, handle it once
      if (lastKeyCode229 && e.keyCode === 13) {
        lastKeyCode229 = null;
