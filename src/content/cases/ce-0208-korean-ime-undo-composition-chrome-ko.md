---
id: ce-0208-korean-ime-undo-composition-chrome-ko
...
keyboard: Korean (IME)
caseTitle: 한글 IME 조합 중 실행 취소가 예상보다 더 많은 텍스트를 지움
description: "contenteditable 요소에서 한글 IME 조합이 활성화된 상태에서 실행 취소를 누르면 예상보다 더 많은 텍스트가 제거되며, 현재 조합뿐만 아니라 이전에 커밋된 문자도 포함됩니다."
tags:
  - undo
  - composition
  - ime
...
`contenteditable` 요소에서 한글 IME 조합이 활성화된 상태에서 실행 취소를 누르면 예상보다 더 많은 텍스트가 제거되며, 현재 조합뿐만 아니라 이전에 커밋된 문자도 포함됩니다.

## 재현 예시

1. 편집 가능한 영역에 포커스를 둡니다.
2. 짧은 단어를 입력하고 완료합니다 (예: "한").
3. 한글 IME를 활성화하고 다른 단어 조합을 시작합니다 (예: "글"), 하지만 완료하지 않습니다.
4. Ctrl+Z(또는 플랫폼별 실행 취소 단축키)를 누릅니다.
...
- **Chrome**: 조합 중 실행 취소가 예상보다 더 많은 것을 제거할 수 있음
- **Edge**: Chrome과 유사함
