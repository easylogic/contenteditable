...
description: "contenteditable 요소에서 한글 IME 조합 중 Enter를 누르면 두 개의 keydown 이벤트가 발생합니다: 첫 번째는 keyCode 229(IME 처리), 두 번째는 keyCode 13(Enter). 이것은 단일 Enter 키 입력에 대해 이벤트 핸들러가 두 번 실행될 수 있습니다."
tags:
  - composition
  - ime
  - enter
...
`contenteditable` 요소에서 한글 IME 조합 중 Enter를 누르면 두 개의 `keydown` 이벤트가 순차적으로 발생합니다:
