---
id: scenario-ime-composition-paste
title: 붙여넣기 작업이 IME 컴포지션을 취소하거나 간섭함
description: "contenteditable 요소에서 IME로 텍스트를 컴포지션할 때 콘텐츠를 붙여넣기(Ctrl+V / Cmd+V)하면 활성 컴포지션이 취소되거나, 컴포지션된 텍스트가 손실되거나, 붙여넣은 콘텐츠가 예상치 못한 위치에 삽입될 수 있습니다. 여러 언어에 영향을 미칩니다."
category: ime
tags:
  - ime
  - composition
  - paste
status: draft
locale: ko
---

`contenteditable` 요소에서 IME로 텍스트를 컴포지션할 때 콘텐츠를 붙여넣기(Ctrl+V / Cmd+V)하면 활성 컴포지션이 취소되거나, 컴포지션된 텍스트가 손실되거나, 붙여넣은 콘텐츠가 예상치 못한 위치에 삽입될 수 있습니다.

## 관찰된 동작

1. **컴포지션 취소**: 붙여넣기 작업이 활성 컴포지션을 취소합니다
2. **텍스트 손실**: 붙여넣기가 발생하면 컴포지션된 텍스트가 손실됩니다
3. **예상치 못한 삽입**: 붙여넣은 콘텐츠가 잘못된 위치에 삽입될 수 있습니다
4. **이벤트 시퀀스 문제**: `paste`, `compositionend`, `input` 이벤트의 시퀀스가 일관되지 않을 수 있습니다
5. **혼합 콘텐츠**: 붙여넣은 콘텐츠와 컴포지션된 텍스트가 잘못 혼합될 수 있습니다

## 언어별 특성

이 문제는 IME 컴포지션을 사용하는 모든 언어에서 나타납니다:

- **한국어 IME**: 붙여넣기가 발생하면 부분 음절이 손실될 수 있습니다
- **일본어 IME**: 불완전한 한자 변환이 손실될 수 있습니다
- **중국어 IME**: 부분 병음 또는 문자 변환이 손실될 수 있습니다
- **기타 IME**: 다른 언어에서도 유사한 문제가 발생할 수 있습니다

## 브라우저 비교

- **Chrome/Edge**: 붙여넣기가 컴포지션을 취소할 수 있습니다
- **Firefox**: 컴포지션 중 붙여넣기 동작이 다를 수 있습니다
- **Safari**: 컴포지션 중 붙여넣기 처리가 일관되지 않을 수 있습니다

## 영향

- 컴포지션 중 붙여넣기를 하면 사용자의 작업이 손실됩니다
- 컴포지션이 예상치 못하게 취소되면 워크플로가 중단됩니다
- 붙여넣은 콘텐츠가 잘못 삽입될 수 있습니다
- 네이티브 입력 필드에 비해 사용자 경험이 저하됩니다

## 해결 방법

컴포지션 중 붙여넣기 이벤트를 주의 깊게 처리합니다:

```javascript
let isComposing = false;

element.addEventListener('compositionstart', () => {
  isComposing = true;
});

element.addEventListener('compositionend', () => {
  isComposing = false;
});

element.addEventListener('paste', (e) => {
  if (isComposing) {
    // 옵션 1: 컴포지션 중 붙여넣기 방지
    e.preventDefault();
    // 옵션 2: 컴포지션이 완료될 때까지 대기한 후 붙여넣기
    // 이것은 붙여넣기 작업을 대기열에 넣는 것을 요구합니다
    
    // 옵션 3: 먼저 컴포지션을 커밋한 후 붙여넣기 허용
    // 이것은 IME 특정 처리를 요구할 수 있습니다
  }
});

// 대안: beforeinput 이벤트 사용
element.addEventListener('beforeinput', (e) => {
  if (e.inputType === 'insertFromPaste' && isComposing) {
    // 컴포지션 중 붙여넣기 처리
    // 기본 동작을 방지하고 수동으로 처리해야 할 수 있습니다
  }
});
```
